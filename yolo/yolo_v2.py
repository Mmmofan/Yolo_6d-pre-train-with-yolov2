...
class yolo_v2(object):
    def __init__(self, isTraining = True):
        ...
        if isTraining:
            ...
            self.labels = tf.placeholder(tf.float32, [self.batch_size, 15], name='labels')
            self.loss_layer_for_pretraining(self.logits, self.labels)
    ...
    def build_networks(self, inputs):
        ...
        # for pre-training in Yolo_6d
        net = self.conv_layer(net, [1, 1, 1024, 1024], batch_norm=True, name='30_conv')
        net = self.flatten(net, name='flatten')
        net = self.fc(net, [net.get_shape().as_list()[1], 15], activation=False, name='full_connect')
    ...
    def flatten(self, inputs, name):
        ...
        for d in shape[1:]:
            dim *= d
        flat_layer = tf.reshape(inputs, [-1, dim], name=name)
        return flat_layer


    def fc(self, inputs, shape, activation=True, name='full_connect'):
        weight = tf.Variable(tf.truncated_normal(shape, stddev=0.1), name='weight')
        biases = tf.Variable(tf.constant(0.1, shape=[shape[1]]), name='biases')
        output = tf.nn.bias_add(tf.matmul(inputs, weight), biases)
        if activation:
    ...
    def loss_layer_for_pretraining(self, predict, label):
